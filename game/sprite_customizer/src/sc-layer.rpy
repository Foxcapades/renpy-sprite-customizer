init -1 python:
    class SCLayer:
        ...
        of those options are then passed to the given `layer_provider` to
        ...
        def __init__(self, name, layer_provider, transform=None, **options):
            ...
            layer_provider (callable|str): Either a Layer Callback used to
            create the displayable that backs this layer, or a string containing
            `{var_name}` variables that will be injected based on the selected
            options.
        ...
        def _render_string(self, st, at, **kwargs):
            ...
            for key in self._options.keys():
                vals[key] = self._options[key]._values[self._state.get_selection(key) - 1]
        ...
        def clone(self):
            ...
            """
            return SCLayer(self._name, self._provider, self._transform, **self._options)
