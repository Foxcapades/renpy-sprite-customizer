...
[NOTE]
--
Options that do not have a group name declared on them, their display name will
...
| `group_order`
| str[]
| Optional list of option group names by which the output `dict` will be
ordered.  This list *MUST* contain all of the groups declared in the SCLayer
...
Randomizes the selections for all the options on this CustomizedSprite instance.
